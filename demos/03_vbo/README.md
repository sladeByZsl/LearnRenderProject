...
### Q: 这次 RenderDoc 截图说明我抓对了吗？

A: 对，截图已经抓到了 Day 03 最关键的链路。
...
- `Vertex Array 49`：RenderDoc 里的 VAO，对应代码里的 `VAO`。
- `Buffer 50`：RenderDoc 里的 VBO，对应代码里的 `VBO`。
- `aPos`：顶点 shader 里的输入变量。
- `R32G32B32_FLOAT`：每个顶点属性是 3 个 32 位 float，也就是 `x, y, z`。
- `stride = 12`：每个顶点占 12 字节，因为 `3 * sizeof(float) = 3 * 4 = 12`。
- `offset = 0`：第一个属性从 buffer 开头读。
- `count = 3`：这次 draw 读取 3 个顶点。
...
这说明 CPU 里的 `vertices[]` 已经成功通过 `glBufferData` 进入 GPU buffer，RenderDoc 能把 draw call 使用的顶点数据还原出来。

最后 Backbuffer 里看到蓝色三角形，说明整条链路成立：

```text
vertices[] -> glBufferData -> VBO(Buffer 50)
VBO + glVertexAttribPointer -> VAO(Vertex Array 49)
VAO + shader -> glDrawArrays(3)
glDrawArrays -> Backbuffer 里的三角形
```

这就是今天 VBO 学习最完整的一次闭环。
